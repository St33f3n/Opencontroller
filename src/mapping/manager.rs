...
impl MappingEngineManager {
    ...
    pub async fn activate_mapping(
        ...
        if let Err(e) = config.validate() {
            ...
            return Err(MappingError::ConfigError(format!("Invalid configuration: {}", e)));
        }
        ...
        let handle = MappingEngineHandle::new(
            mapping_type,
            config_name.clone(),
            task_handle,
            shutdown_tx,
        ...
        info!("Mapping engine activated: {} ({})", config_name, mapping_type);
        Ok(())
    }
