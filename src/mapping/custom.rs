...
impl crate::mapping::MappingConfig for CustomConfig {
    fn validate(&self) -> Result<(), MappingError> {
        ...
        if self.joystick_handlers.is_empty() && self.trigger_handlers.is_empty() && self.button_handlers.is_empty() {
    ...
    fn get_description(&self) -> String {
        format!("Custom mapping for {} protocol v{}", 
...
impl MappingStrategy for CustomStrategy {
    ...
    fn initialize(&mut self) -> Result<(), MappingError> {
        ...
        info!("Protocol: {} v{}", 
            self.config.protocol_config.protocol_name,
            self.config.protocol_config.protocol_version
        );
    ...
    fn shutdown(&mut self) {
        info!("Shutting down custom mapping strategy: {}", self.config.name);
    }
