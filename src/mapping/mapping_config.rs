...
use crate::controller::controller::{
    ButtonEventState, ButtonType, ControllerOutput, JoystickType, TriggerType,
};
use crate::mapping::mapping_strategy::{
    ButtonMappingStrategy, DirectMappingStrategy, MappingStrategy, RegionMappingStrategy,
    ThresholdMappingStrategy,
};
use eframe::egui::Key;
use eframe::egui::Event;
use crate::mapping::mapping_types::{InputComponent, MappedEvent, Region};
...
pub struct KeyboardMappingConfig {
    name: String,
    button_mappings: HashMap<ButtonType, Event>,
...
impl KeyboardMappingConfig {
    ...
    pub fn map_button(&mut self, button: ButtonType, key: Event) -> &mut Self {
    ...
    fn map(&self, input: &ControllerOutput) -> Vec<MappedEvent> {
        ...
        for (trigger_type, strategy) in &self.trigger_mappings {
            match trigger_type {
                TriggerType::Left => {
                    let component =
                        InputComponent::Trigger(*trigger_type, input.left_trigger.value);
                ...
                TriggerType::Right => {
                    let component =
                        InputComponent::Trigger(*trigger_type, input.right_trigger.value);
...
impl ELRSMappingConfig {
    ...
    pub fn add_joystick_mapping(
        ...
        if channel >= self.num_channels {
            warn!(
                "Channel {} exceeds configured channel count {}",
    ...
    pub fn default_quadcopter() -> Self {
        ...
        config.add_joystick_mapping(JoystickType::Right, "y", 1, (-1.0, 1.0), (1000, 2000), true);

        // Throttle (linker Stick Y) -> Kanal 3
        config.add_joystick_mapping(JoystickType::Left, "y", 2, (-1.0, 1.0), (1000, 2000), true);

        // Yaw (linker Stick X) -> Kanal 4
        config.add_joystick_mapping(JoystickType::Left, "x", 3, (-1.0, 1.0), (1000, 2000), false);

        // Arming (Button A) -> Kanal 5
        config.add_button_mapping(ButtonType::A, 4, (1000, 2000));

        // Flugmodus (Button B) -> Kanal 6
        config.add_button_mapping(ButtonType::B, 5, (1000, 2000));
                ...
                InputComponent::Button(button_type, _) => {
                    ...
                    let is_pressed = input.button_events.iter().any(|event| {
                        event.button == *button_type
                            && (event.state == ButtonEventState::Held
                                || event.state == ButtonEventState::Complete)
                    });

                    if is_pressed {
                        // Komponente mit aktuellem Zustand erstellen
                        let component =
                            InputComponent::Button(button_type.clone(), ButtonEventState::Held);
